...
function cekLogin() {
    ...
    if(email === 'Universitas@Pamulang.com' && sandi === 'Mahasiswa') {
        ...
        initGrafik();
        muatDataTersimpan(); // ➕ Muat data otomatis setelah login
    } else {
...
// ➕ PENYIMPANAN OTOMATIS AGAR DATA TIDAK HILANG
function simpanDataOtomatis() {
    localStorage.setItem("dataMahasiswa", JSON.stringify(dataMahasiswa));
}

function muatDataTersimpan() {
    const dataDariBrowser = localStorage.getItem("dataMahasiswa");
    if (dataDariBrowser) {
        dataMahasiswa = JSON.parse(dataDariBrowser);
        tampilkanSemuaData();
...
function updateMahasiswa() {
    ...
    tampilkanSemuaData();
    tampilkanDataLaporan();
    simpanDataOtomatis(); // ➕ Tambah simpan otomatis
}
